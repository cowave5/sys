<template>
  <div class="app-container">
    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch">
      ...
      <el-form-item>
        <el-button type="primary" icon="el-icon-search" size="mini" @click="handleQuery">{{ $t('commons.button.search') }}
        </el-button>
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-close" size="mini" @click="handleClose">{{ $t('commons.button.close') }}
        </el-button>
      </el-col>
    ...
    <el-table ref="table" :data="list" @selection-change="selectRow" v-loading="loading">
      ...
      <el-table-column :label="$t('dept.text.default_user_post')" align="center">
        <template slot-scope="{row: {isDefault}}">
          <template v-for="item in dict.type.sys_yes_no">
            <span v-if="isDefault === item.value">
              <el-tag v-if="$i18n.locale==='zh'" :type="item.css">{{ item.label }}</el-tag>
              <el-tag v-if="$i18n.locale==='en'" :type="item.css">{{ item.labelEn }}</el-tag>
            </span>
          </template>
        </template>
      </el-table-column>
...
export default {
  ...
  dicts: ['post_type', 'post_level', 'sys_yes_no'],
  data() {
    return {
      // 遮罩层
      loading: false,
