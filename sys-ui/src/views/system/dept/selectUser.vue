  ...
  <el-dialog :title="$t('post.button.select')" :visible.sync="visible" width="900px" top="5vh" append-to-body>
    ...
    <el-row>
      <el-table ref="table" :data="list" @selection-change="selectRow" :header-cell-style="{'text-align':'center'}" height="560px">
        ...
        <el-table-column :label="$t('user.label.rank')" prop="rank" align="center" :show-overflow-tooltip="true">
        ...
        <el-table-column :label="$t('dept.text.default_user_post')" prop="isDefault" align="center">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isDefault" :active-value=1 :inactive-value=0 @change="(val)=>handleDefaultPost(val, scope.row)"/>
          </template>
        </el-table-column>
        <el-table-column :label="$t('dept.text.leader')" prop="isLeader" align="center" width="150">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.isLeader" :active-value=1 :inactive-value=0 @change="(val)=>handleDeptLeader(val, scope.row)"/>
          </template>
        </el-table-column>
      </el-table>
    ...
    <div slot="footer" class="dialog-footer">
      <el-button type="primary" @click="handleSubmit" :disabled="!checkPermit(['sys:dept:positions'])">
