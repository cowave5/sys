<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button type="success" plain icon="el-icon-edit" size="mini" @click="handleUpdate"
                   :disabled="single || !checkPermit(['sys:dict:edit'])">{{$t('commons.button.edit')}}</el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <el-table v-loading="loading" :data="typeList" @selection-change="handleSelectionChange" :header-cell-style="{'text-align':'center'}">
      ...
      <el-table-column :label="$t('commons.label.status')" prop="status" align="center" width="100" >
        <template slot-scope="scope">
          <dict-tag :options="dict.type.sys_is_enable" :value="scope.row.status"/>
        </template>
      </el-table-column>
      <el-table-column :label="$t('commons.label.readonly')" prop="readOnly" align="center" width="100" >
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('commons.label.options')" align="center" width="120" class-name="small-padding fixed-width">
        <template slot-scope="scope">
