      ...
      <el-table-column :label="$t('commons.label.options')" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          ...
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     :disabled="scope.row.readOnly === 1 || !checkPermit(['sys:menu:delete'])">{{$t('commons.button.delete')}}</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="700px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm"
                   :disabled="form.readOnly === 1 || !checkPermit(['sys:menu:edit'])">{{$t('commons.button.confirm')}}</el-button>
...
<script>
import { getMenuList, getMenuInfo, delMenu, addMenu, updateMenu, changeReadonly } from "@/api/system/menu";
  ...
  methods: {
    ...
    handleReadOnly(row) {
      let text = row.readOnly === 1
          ? this.$t('menu.confirm.readonly_set', { arg1: this.$t(row.menuName) })
          : this.$t('menu.confirm.readonly_cancel', { arg1: this.$t(row.menuName) });
      this.$modal.confirm(text).then(function() {
        return changeReadonly(row.menuId, row.readOnly);
      }).then(() => {
