    ...
    <el-row :gutter="20">
      ...
      <el-col :span="4" :xs="24">
        <div class="head-container">
          <el-input v-model="deptName" :placeholder="$t('dept.placeholder.name')" clearable size="small" prefix-icon="el-icon-search" style="margin-bottom: 20px"/>
        </div>
        <div class="head-container">
      ...
      <el-col :span="20" :xs="24">
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                       :disabled="!checkPermit(['sys:post:create'])">{{$t('commons.button.create')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="success" plain icon="el-icon-edit" size="mini" @click="handleUpdate"
                       :disabled="single || !checkPermit(['sys:post:edit'])">{{$t('commons.button.edit')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="el-icon-delete" size="mini" @click="handleDelete"
                       :disabled="multiple || !checkPermit(['sys:post:delete'])">{{$t('commons.button.delete')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport"
                       :disabled="!checkPermit(['sys:post:export'])">{{$t('commons.button.export')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="primary" plain  size="mini" @click="showDiagram"
                       :disabled="!checkPermit(['sys:post:diagram'])"><svg-icon icon-class="tree"/>{{$t('commons.button.diagram')}}</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button type="danger" plain icon="el-icon-refresh" size="mini" @click="handleRefreshCache"
                       :disabled="!checkPermit(['sys:post:cache'])">{{$t('commons.button.cache')}}</el-button>
          </el-col>
        ...
        <el-table v-loading="loading" :data="postList" @selection-change="handleSelectionChange">
          ...
          <el-table-column :label="$t('commons.label.status')" align="center" prop="postStatus">
            <template slot-scope="scope">
              <dict-tag :options="dict.type.sys_is_enable" :value="scope.row.postStatus"/>
            </template>
          </el-table-column>
          <el-table-column :label="$t('commons.label.remark')" align="center" prop="remark"/>
          ...
          <el-table-column :label="$t('commons.label.options')" align="center" class-name="small-padding fixed-width">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                         :disabled="scope.row.readOnly === 1 || !checkPermit(['sys:post:delete'])">{{$t('commons.button.delete')}}</el-button>
            </template>
          </el-table-column>
        </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="submitForm"
                   :disabled="form.readOnly === 1 || !checkPermit(['sys:post:edit'])">{{$t('commons.button.confirm')}}</el-button>
...
<script>
import {listPost, getPost, delPost, addPost, updatePost, getPostDiagram, refreshDiagram, changeReadonly} from "@/api/system/post";
  ...
  methods: {
    ...
    handleReadOnly(row) {
      let text = row.readOnly === 1
          ? this.$t('post.confirm.readonly_set', { arg1: row.postName })
          : this.$t('post.confirm.readonly_cancel', { arg1: row.postName });
