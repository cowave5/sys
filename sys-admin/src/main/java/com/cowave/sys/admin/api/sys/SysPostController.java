...
import com.cowave.sys.admin.service.sys.SysPostService;
import com.cowave.sys.admin.core.cache.SysPostCache;
...
public class SysPostController {
	...
	private final SysPostCache sysPostCache;
	...
	public Response<Void> refresh() throws Exception {
		sysPostCache.refreshTree();
		sysPostCache.refreshDeptPostTree();
	...
	public Response<Tree<String>> tree() {
		return Response.success(sysPostCache.tree());
	}
	...
	public Response<List<Tree<String>>> deptPostTree() {
		return Response.success(List.of(sysPostCache.deptPostTree()));
	}
	 ...
	 * 新增
	 */
	@Operation(type = "admin_post", action = "add", desc = "新增岗位：#{#sysPost.postName}", handleExpr = "#opHandler.logRequest(#opInfo)")
	 ...
	 * 修改
	 */
	@Operation(type = "admin_post", action = "edit", desc = "修改岗位：#{#resp.postName}", handleExpr = "#opHandler.logAll(#opInfo, #sysPost, #resp)")
	 ...
	 * @param postId 岗位id
	 */
	@Operation(type = "admin_post", action = "delete", desc = "删除岗位", handleExpr = "#opHandler.logResponse(#opInfo, #resp)")
	 ...
	 * 修改只读
	 */
	@Operation(type = "admin_post", action = "readonly", desc = "修改岗位[#{#sysPost.postName}]只读状态", handleExpr = "#opHandler.logRequest(#opInfo)")
