...
import com.cowave.sys.admin.service.sys.SysUserService;
import com.cowave.sys.admin.core.cache.SysUserCache;
...
public class SysUserController {
	...
	public Response<Void> refresh() throws Exception {
		sysUserCache.refreshTree();
		sysUserCache.refreshDeptUserTree();
	...
	public Response<Tree<String>> tree() {
		return Response.success(sysUserCache.tree());
	}
	...
	public Response<List<Tree<String>>> deptUserTree() {
		return Response.success(List.of(sysUserCache.deptUserTree()));
	}
	 ...
	 * 新增
	 */
	@Operation(type = "admin_user", action = "add", desc = "新增用户：#{#sysUser.userName}", handleExpr = "#opHandler.logRequest(#opInfo)")
	 ...
	 * 编辑
	 */
	@Operation(type = "admin_user", action = "edit", desc = "修改用户：#{#sysUser.userName}", handleExpr = "#opHandler.logAll(#opInfo, #sysUser, #resp)")
	 ...
	 * @param userId 用户id
	 */
	@Operation(type = "admin_user", action = "delete", desc = "删除用户", handleExpr = "#opHandler.logResponse(#opInfo, #resp)")
	 ...
	 * 修改状态
	 */
	@Operation(type = "admin_user", action = "status", desc = "修改用户[#{#sysUser.userName}]状态", handleExpr = "#opHandler.logRequest(#opInfo)")
	@PreAuthorize("@permit.hasPermit('sys:user:edit')")
	 ...
	 * 修改密码
	 */
	@Operation(type = "admin_user", action = "passwd", desc = "修改用户[#{#sysUser.userName}]密码", handleExpr = "#opHandler.log(#opInfo)")
	 ...
	 * 修改角色
	 */
	@Operation(type = "admin_user", action = "role_grant", desc = "修改用户[#{#sysUser.userName}]角色", handleExpr = "#opHandler.log(#opInfo)")
	 ...
	 * 修改只读
	 */
	@Operation(type = "admin_user", action = "readonly", desc = "修改用户[#{#sysUser.userName}]只读状态", handleExpr = "#opHandler.logRequest(#opInfo)")
