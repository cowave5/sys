...
public class SysUserDao extends ServiceImpl<SysUserMapper, SysUser> {
    ...
    public boolean isAccountExist(String userAccount, Long userId){
    ...
    public void updateUserPasswd(Long userId, String passwd){
        LambdaUpdateWrapper<SysUser> updateWrapper = prepareUpdateWrapper(userId);
        updateWrapper.set(SysUser::getUserPasswd, passwd);
        update(updateWrapper);
    }

    public void updateUserReadonly(Long userId, Integer readonly){
        LambdaUpdateWrapper<SysUser> updateWrapper = prepareUpdateWrapper(userId);
        updateWrapper.set(SysUser::getReadOnly, readonly);
        update(updateWrapper);
    }

    private LambdaUpdateWrapper<SysUser> prepareUpdateWrapper(Long userId){
        Long updateUserId = Optional.ofNullable(Access.userId())
                .map(v -> ((Number)v).longValue()).orElse(null);
        Long updateDeptId = Optional.ofNullable(Access.deptId())
                .map(v -> ((Number)v).longValue()).orElse(null);
        LambdaUpdateWrapper<SysUser> updateWrapper = new LambdaUpdateWrapper<>();
        updateWrapper.eq(SysUser::getUserId, userId);
        updateWrapper.set(SysUser::getUpdateUser, updateUserId);
        updateWrapper.set(SysUser::getUpdateDept, updateDeptId);
