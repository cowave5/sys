...
public class SysUserServiceImpl implements SysUserService {
	...
	private UserInfoDto delete(Integer userId){
		...
		HttpAsserts.notEquals(1, preUser.getReadOnly(), FORBIDDEN, "{user.forbid.delete.readonly}");
		// 删除用户
	...
	private UserInfoDto getAndCheckEditable(Integer userId){
		...
		HttpAsserts.notEquals(1, preUser.getReadOnly(), FORBIDDEN, "{user.forbid.edit.readonly}", preUser.getUserAccount());
		return preUser;
	}
	...
	public void changeStatus(UserStatusUpdate statusUpdate) {
		...
		sysUserDao.updateStatusById(statusUpdate.getUserId(), statusUpdate.getUserStatus());
	}
	...
	public void changePasswd(UserPasswdUpdate passwdUpdate) {
		...
		sysUserDao.updatePasswdById(passwdUpdate.getUserId(), newPasswd);
	}
