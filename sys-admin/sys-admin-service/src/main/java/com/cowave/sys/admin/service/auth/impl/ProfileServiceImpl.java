...
import static com.cowave.sys.admin.domain.rabc.enums.AccessType.OAUTH_GITLAB;
...
public class ProfileServiceImpl implements ProfileService {
    ...
    public UserProfile info() throws Exception {
        ...
        if(OAUTH_GITLAB.isTypeEquals(userCode)){
            return oauthUserDtoMapper.getOauthProfile(Access.userId());
        }else if(LDAP.isTypeEquals(userCode)){
            return ldapUserDtoMapper.getLdapUserProfile(Access.userId());
        }else{
            UserProfile userProfile = sysUserDtoMapper.getUserProfile(Access.userId());
            SysAttach avatar = attachService.latestOfMaster(Long.valueOf(Access.userId()), "sys-user");
            if(avatar != null){
