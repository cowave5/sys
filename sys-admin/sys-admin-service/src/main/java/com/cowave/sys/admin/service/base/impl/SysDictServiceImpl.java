...
import com.cowave.sys.admin.domain.base.request.DictReadUpdate;
...
public class SysDictServiceImpl implements SysDictService {
	...
	public void delete(List<Integer> dictIds) {
		long readonlyCount = sysDictDao.countReadOnlyByIds(dictIds);
		HttpAsserts.isTrue(readonlyCount == 0, FORBIDDEN, "{dict.forbid.delete.readonly}");
	...
	public void edit(DictInfoDto sysDict){
		HttpAsserts.notNull(sysDict.getId(), BAD_REQUEST, "{dict.notnull.id}");

		DictInfoDto preDict = sysDictMapper.getById(sysDict.getId());
		HttpAsserts.notNull(preDict, NOT_FOUND, "{dict.notexist}", sysDict.getId());
		HttpAsserts.notEquals(1, preDict.getReadOnly(), FORBIDDEN, "{dict.forbid.edit.readonly}");
	...
	@Override
	public void updateReadonly(DictReadUpdate dictUpdate) {
		sysDictDao.updateReadonly(dictUpdate.getDictId(), dictUpdate.getReadOnly());
	}
