...
public class CaptchaService {
    ...
    private static final String CAPTCHA_KEY = "admin:captcha:";
    ...
    public CaptchaInfo captcha() throws IOException {
        List<OAuthConfig> oauthList = oAuthService.queryEnabledList();
        String oauthUrl = null;
        if(CollectionUtils.isNotEmpty(oauthList)){
            // 目前只有Gitlab一个授权服务
            OAuthConfig oauthConfig = oauthList.get(0);
            if(GITLAB.isEqual(oauthConfig.getAppType())){
                oauthUrl = GITLAB.getAuthUrl(oauthConfig);
            }
        }
