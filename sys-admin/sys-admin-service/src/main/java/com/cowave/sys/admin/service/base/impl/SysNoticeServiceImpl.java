...
import static com.cowave.sys.admin.domain.rabc.enums.UserType.*;
...
public class SysNoticeServiceImpl implements SysNoticeService {
    ...
    public Response.Page<NoticeDto> list(NoticeQuery query) {
        ...
        for(SysNotice sysNotice : page.getRecords()){
            ...
            }else if(OAUTH_GITLAB.isTypeEquals(userCode)){
    ...
    private void delete(Long noticeId) throws Exception {
        ...
        if(!Access.isAdminUser()){
            HttpAsserts.equals(notice.getCreateBy(), Access.userCode(), FORBIDDEN, "{notice.only.delete.self}");
        }
    ...
    public void edit(NoticeCreate sysNotice) throws Exception {
        HttpAsserts.notNull(sysNotice.getNoticeId(), BAD_REQUEST, "{notice.notnull.id}");

        SysNotice notice = sysNoticeDao.getById(sysNotice.getNoticeId());
        HttpAsserts.notNull(notice, NOT_FOUND, "{notice.notexist}", sysNotice.getNoticeId());
        HttpAsserts.isTrue(DRAFT.isEqual(notice.getNoticeStatus()), BAD_REQUEST, "{notice.only.edit.unpublish}");

        if(!Access.isAdminUser()){
            HttpAsserts.equals(notice.getCreateBy(), FORBIDDEN, Access.userCode(), "{notice.only.edit.self}");
        }
    ...
    public void publish(Long noticeId) {
        SysNotice notice = sysNoticeDao.getById(noticeId);
        HttpAsserts.notNull(notice, NOT_FOUND, "{notice.notexist}", noticeId);
        if(!Access.isAdminUser()){
            HttpAsserts.equals(notice.getCreateBy(), Access.userCode(), FORBIDDEN, "{notice.only.publish.self}");
        }
        ...
        socketIoHelper.sendClients(userCodes, "sys_notice", notice.getNoticeTitle());
    }
