...
public class SysPostDao extends ServiceImpl<SysPostMapper, SysPost> {
    ...
    public long countReadOnlyByPostIds(List<Integer> postIds){
        return lambdaQuery().eq(SysPost::getReadOnly, 1).in(SysPost::getPostId, postIds).count();
    }
    ...
    public void updatePost(SysPost sysPost){
        lambdaUpdate().eq(SysPost::getPostId, sysPost.getPostId())
                .set(SysPost::getUpdateUser, Access.userId())
                .set(SysPost::getUpdateDept, Access.deptId())
     ...
     * 修改岗位只读
     */
    public void updateReadonly(Integer postId, Integer readonly){
        lambdaUpdate().eq(SysPost::getPostId, postId)
                .set(SysPost::getUpdateUser, Access.userId())
                .set(SysPost::getUpdateDept, Access.deptId())
                .set(SysPost::getUpdateTime, new Date())
                .set(SysPost::getReadOnly, readonly)
                .update();
    }
}
