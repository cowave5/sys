...
@Component
public class SysOperationKafka {

    private final SysOperationHandler operationHandler;

    @KafkaListener(topics = {"${spring.access.oplog-kafka:sys-oplog}"})
    public void consume(ConsumerRecord<?, ?> message) {
        operationHandler.create(JSON.parseObject(String.valueOf(message.value()), SysOperation.class));
    }
}
