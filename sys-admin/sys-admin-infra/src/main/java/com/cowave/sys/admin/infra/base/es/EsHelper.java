package com.cowave.sys.admin.infra.base.es;
...
public class EsHelper {
    ...
    public <T> Response.Page<T> query(String index, SearchSourceBuilder searchSourceBuilder, Class<T> clazz){
        SearchRequest searchRequest = new SearchRequest(index);
        searchRequest.source(searchSourceBuilder);
        searchRequest.indicesOptions(IndicesOptions.lenientExpandOpen());
        try {
            ...
            for (SearchHit hit : hits) {
                list.add(objectMapper.readValue(hit.getSourceAsString(), clazz));
            }
    ...
    public <T> void indexSetting(String index, Settings.Builder builder) {
        try {
            if (indexExist(index)) {
